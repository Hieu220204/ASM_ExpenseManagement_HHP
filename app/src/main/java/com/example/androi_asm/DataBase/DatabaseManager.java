...
//        }
    ...
    public boolean updateExpenseReport(int reportID, String content, String date) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("content", content);
        values.put("date", date);
        return db.update("Reports", values, "reportID=?", new String[]{String.valueOf(reportID)}) > 0;
    }

    public boolean deleteExpenseReport(int reportID) {
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete("Reports", "reportID=?", new String[]{String.valueOf(reportID)}) > 0;
    }
